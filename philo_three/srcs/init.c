...
/*   Updated: 2020/10/12 16:09:04 by rchallie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
)
{
	...
	(argc == 6) ? (simulation->each_must_eat = ft_atolu(argv[5])) : 0;
	simulation->start_time = get_actual_time();
	if ((simulation->philosophers = init_philosophers(
		simulation, ft_atolu(argv[1]))) == NULL ||
		(simulation->philos_pid = create_philosophers_fork(
		simulation->philosophers, simulation->number_of_philosopher)) == NULL)
		return (NULL);
	return (simulation);
}
